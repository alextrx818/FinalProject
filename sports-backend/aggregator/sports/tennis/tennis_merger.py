...
class TennisMerger:
    ...
    def extract_odds(self, data, source='bets'):
        """Extract odds from the complex data structure."""
        if source == 'bets':
            if isinstance(data, dict):
                # Try to find odds in the match winner market
                markets = data.get('markets', {})
                match_winner = markets.get('match_winner', {})
                odds = match_winner.get('odds', [])
                if odds and len(odds) > 0:
                    return odds[0].get('odds')
        else:
            if isinstance(data, dict):
                event_data = data.get('raw_event_data', {})
                return event_data.get('odds')
        return None
    ...
    def merge(
        ...
        matched_rapid_indices = set()
        matched_bets_indices = set()
        ...
        for rapid_idx, rd in enumerate(rapid_data):
            ...
            # Extract status from RapidAPI data
            ...
            for bets_idx, bd in enumerate(bets_data):
                ...
                if rapid_ids and bets_possible_ids[bets_idx]:
                    ...
                    if overlap:
                        ...
                        # Extract odds
                        pre_match_odds = self.extract_odds(bd, 'bets')
                        live_odds = self.extract_odds(rd, 'rapid')
