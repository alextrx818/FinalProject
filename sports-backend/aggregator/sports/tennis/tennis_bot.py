...
class TennisBot:
    ...
    def __init__(
        ...
        self.merger = tennis_merger.TennisMerger()  # Create an instance of TennisMerger
    ...
    async def run(self) -> None:
        ...
        while True:
            ...
            try:
                ...
                merged_data = self.merger.merge(bets_data, rapid_data)
                
                # Get and log matching statistics as part of the regular fetch
                stats = self.merger.get_match_stats(merged_data)
                logger.info("\n=== Tennis Match Statistics (Updated: {}) ===".format(
                    datetime.now().strftime("%Y-%m-%d %H:%M:%S")
                ))
                logger.info(f"Total events fetched:")
                logger.info(f"  - BetsAPI: {len(bets_data)}")
                logger.info(f"  - RapidAPI: {len(rapid_data)}")
                logger.info(f"Successfully matched events: {stats['successful_matches']}")
                logger.info(f"Fuzzy matches: {stats['fuzzy_matches']}")
                logger.info("Unmatched events:")
                logger.info(f"  - BetsAPI only: {stats['unmatched_bets']}")
                logger.info(f"  - RapidAPI only: {stats['unmatched_rapid']}")
                logger.info("==========================================\n")
